...
#
# Terraform cannot create its own backend — the bucket must exist before init —
# so this is the one piece of infrastructure that has to be provisioned outside
# it. terraform/main.tf declares a partial `backend "s3" {}` and takes every
# value from the generated file.
#
# Idempotent: existing bucket and table are reused, and the file is rewritten
# with the same content, so this is safe on every run.
#
# Args: <region> <cluster-name>
ensure_tf_state_backend() {
  local region="$1" cluster="$2"
  local account bucket table hcl

  account=$(aws sts get-caller-identity --query Account --output text 2>/dev/null) \
    || err "Cannot read the AWS account id — run 'aws configure' (or set AWS_PROFILE) and retry."

  # Bucket names are globally unique across all of AWS, so the account id is not
  # decoration — without it the first two people to try this template collide.
  #
  # This pattern deliberately reproduces the names the old hardcoded backend used
  # (idp-mvp-terraform-state-<account> / idp-mvp-terraform-locks). Any existing
  # install therefore resolves to the bucket its state is already in; a different
  # naming scheme would init an empty backend, read no state, and re-create the
  # entire VPC and cluster alongside the running one.
  bucket="${cluster}-terraform-state-${account}"
  table="${cluster}-terraform-locks"
  hcl="${ROOT_DIR}/terraform/backend.hcl"

  if aws s3api head-bucket --bucket "$bucket" &>/dev/null; then
    log "  Terraform state bucket s3://${bucket} already exists."
  else
    log "  Creating Terraform state bucket s3://${bucket} in ${region}..."
    ...
    if [[ "$region" == "us-east-1" ]]; then
      aws s3api create-bucket --bucket "$bucket" --region us-east-1 >/dev/null \
        || err "Could not create s3://${bucket}. The name may be taken in another account, or your credentials lack s3:CreateBucket."
    else
    ...
    log "  Created s3://${bucket} (versioned, SSE-S3, public access blocked)."
  fi

  if aws dynamodb describe-table --table-name "$table" --region "$region" &>/dev/null; then
    log "  Terraform lock table ${table} already exists."
  else
    ...
    aws dynamodb create-table --table-name "$table" --region "$region" \
      ...
      || err "Could not create the DynamoDB lock table ${table} — check your credentials have dynamodb:CreateTable."
    aws dynamodb wait table-exists --table-name "$table" --region "$region"
    log "  Created ${table}."
  fi

  cat > "$hcl" <<EOF
# Generated by scripts/setup.sh — gitignored, do not commit.
