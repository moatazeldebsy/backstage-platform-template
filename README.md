...
| **LiteLLM** (spend dashboard) | http://litellm.idp.local (also surfaced in Backstage as the **LiteLLM Spend** page) | installed by default by `bootstrap-ai.sh` (`--skip-litellm` opts out); the nav item stays hidden until LiteLLM is installed |
