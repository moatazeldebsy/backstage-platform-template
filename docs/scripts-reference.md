...
| `--litellm` | off (local) / on (`--aws`) | Deploy LiteLLM as the model backend behind the AI Gateway — Anthropic + Bedrock through one proxy, with virtual keys and spend tracking. Off by default locally: it's a second proxy process on a cluster already measured to have little headroom (see [Local Setup](local-setup.md)). On by default on AWS, since Bedrock only exists there. Without it, the AI Gateway's tools still work but model calls fail upstream. Requires `LITELLM_MASTER_KEY`. See [ADR-0008](design/adr-0008-litellm-multiprovider-gateway.md). |
| `--skip-litellm` | — | Force LiteLLM off even on `--aws`. |
