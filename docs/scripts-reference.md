...
| `run-exporters-from-host.sh` | Runs the DORA / flaky-test / tech-insights exporters from the host against the cluster's Pushgateway, for when the cluster's egress to GitHub is failing but the host's is fine. Reads every env var from the live CronJob so it cannot drift; `--list`, `--dry-run`, `--loop N` |
...
| `run-exporters-from-host.sh` | Runs an exporter from the host against the cluster's Pushgateway. Exists because this machine's Kind cluster cannot reliably reach GitHub — the same API call answering in ~0.6s on the host raises `ConnectionError`/`SSLError` in a pod — which leaves Developer Experience, Reliability and Quality unscored. Reads every environment variable from the live CronJob spec and only rewrites in-cluster DNS to the ingress, probing each address first, so it cannot drift from what the cluster runs. Pushgateway is in-memory, so `--loop N` re-pushes after a restart. | Manual, while cluster egress is broken |
