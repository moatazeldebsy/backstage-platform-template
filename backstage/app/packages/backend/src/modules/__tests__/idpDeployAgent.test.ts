...
import { createDeployAgentAction } from '../idpDeployAgent';
...
describe('idp:deploy-agent', () => {
  const action = createDeployAgentAction();
  ...
  it('checks cluster reachability before writing or applying anything', async () => {
    ...
    const { ctx } = makeCtx({ name: 'my-agent', description: 'test agent' });
  ...
  it('applies the CRD via execFile with an args array, not a shell string', async () => {
    ...
    const { ctx, outputs } = makeCtx({ name: 'my-agent', description: 'test agent' });
    ...
    expect(filename).toBe('my-agent.yaml');

    const applyCall = mockExecFile.mock.calls.find(c => c[1][0] === 'apply');
    expect(applyCall).toBeDefined();
    // Array args, not a single interpolated string — this is what closes the
    // shell-injection risk that used to exist via `name` in the tmpFile path.
    expect(applyCall![1]).toEqual(['apply', '-f', '/tmp/mock-secure-dir/my-agent.yaml']);
    expect(typeof applyCall![1]).not.toBe('string');
    expect(outputs.agentUrl).toBeDefined();
  });
