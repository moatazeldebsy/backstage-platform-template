...
describe('idp:create-namespace', () => {
  ...
  it('applies the manifest and reports the namespace as output', async () => {
    succeedOn(() => true);
    ...
    expect(applyCall![1]).toEqual(['apply', '-f', expect.stringContaining('namespace-scratch-payments-')]);
    expect(outputs.namespace).toBe('scratch-payments');
  });

  it('always cleans up the temp manifest file, even when kubectl apply fails', async () => {
    mockExecFile.mockImplementation((_cmd: string, args: string[], _opts: any, cb: any) => {
  ...
  it('does not fail the action if cleanup itself fails (best-effort)', async () => {
    succeedOn(() => true);
  ...
  it('applies the default-deny NetworkPolicy unless explicitly disabled', async () => {
    ...
    const { ctx } = makeCtx({ name: 'scratch-payments' });
    await action.handler(ctx);
    expect(mockWriteFile.mock.calls[0][1] as string).toContain('kind: NetworkPolicy');
  });
  ...
  it('omits the NetworkPolicy when networkPolicy: false', async () => {
    ...
    await action.handler(ctx);
    expect(mockWriteFile.mock.calls[0][1] as string).not.toContain('kind: NetworkPolicy');
  });
