...
This service was scaffolded via the IDP golden path (JVM/Spring Boot template).

| Property | Value |
|----------|-------|
| Owner | `${{ values.owner }}` |
| Port | `${{ values.port }}` |
| Language | Java 21 / Spring Boot / Gradle |
...
Push to main (this repo)
  └─▶ GitHub Actions (.github/workflows/ci.yml)
        ├─ checkstyle
        ├─ gradle test (70% coverage gate via Jacoco)
...
├── src/
│   ├── main/java/io/idp/service/
│   │   ├── Application.java
│   │   └── ServiceController.java
│   ├── main/resources/application.yml
│   └── test/java/io/idp/service/ServiceControllerTests.java
├── build.gradle.kts
├── settings.gradle.kts
├── Dockerfile
...
If this service replaces a module of the Ruby monolith, register the module boundary in
`docs/monolith-decomposition.md` (platform repo) and keep the contract-testing gate
(`.github/workflows/contract-check.yml`) enabled so the monolith and this service can evolve
independently without breaking each other silently.
