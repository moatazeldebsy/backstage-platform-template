...
```json
{"status":"ready","model":"${{ values.model }}","llm_base_url":"http://ai-gateway.ml-platform.svc.cluster.local:3000","tracing_enabled":true}
```

`llm_base_url` shows where model calls go; `tracing_enabled: false` points at the
`langfuse-otel` secret — see below.

## `/chat` returns 503, `/healthz` is fine

Model calls are not getting through. This service holds no provider key, so the cause is never a
